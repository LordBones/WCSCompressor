    ...
    public class Trie<T>
    {
        ...
        private TrieNode InsertRestNodes(TrieNode startNode, byte[] key)
        {
            ...
            if(node == null)
            {
                ...
                this._root[key[currentDepth]] = node;
            }
            else
            {
            ...
            while(currentDepth < key.Length-1)
            {
                if (node.Children == null)
                {
                    ...
                    currentDepth++;
                    node.Children[0] = new ChildrenItem(key[currentDepth],new TrieNode(key[currentDepth], currentDepth));
                ...
                node.Children = InsertIntoChildren(node.Children, i, new ChildrenItem(key[currentDepth], newNode));
